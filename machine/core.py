...
class Machine:
    ...
    def load_plugins(self):
        ...
        self._loop.run_until_complete(self._storage.set("manual", dill.dumps(self._help)))
    ...
    async def run(self):
        ...
        with indent(4):
            teardowns = []
            try:
                teardowns, exceptions = await aio.split([
                    self._connect_slack(),
                ...
                print("TEARDOWNS", teardowns)
                for exception in exceptions:
                    raise exception
            except (KeyboardInterrupt, SystemExit):
                announce("\nTearing down Slack Machine...")
                for teardown in teardowns:
                    if teardown is None:
                        continue

                    if isinstance(teardown, list):
                        await aio.join(teardown)
                    else:
                        await teardown

    async def _connect_slack(self):
        connected = await self._client.rtm.start()
        if not connected:
    ...
    async def _start_keepaliver(self):
        ...
        if interval:
            show_valid(f"Keepalive coroutine starting [Interval: {interval}s]")
